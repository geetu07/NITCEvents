...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const selectedImage = document.querySelector(".sel-img img");
  
    // Select all the images in the sidebox
    const sideImages = document.querySelectorAll(".sidebox .img-box img");
  
    // Loop through all sidebox images
    sideImages.forEach((image) => {
      // Add a click event listener to each image
      image.addEventListener("click", () => {
        // Change the src attribute of the selected image
        selectedImage.src = image.src;
  
        const description = document.querySelector(".sel-img .img-desc");
        description.textContent = image.alt; // Use the 'alt' text for the description
      });
    });
  });